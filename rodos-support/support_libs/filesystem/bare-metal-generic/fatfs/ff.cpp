...
#include "fatTime.h"	/* FatTime services */
...
static
int32_t mem_cmp (const void* dst, const void* src, UINT cnt) {
...
static
int32_t chk_chr (const char* str, int32_t chr) {
...
static
int32_t enq_lock (void)	/* Check if an entry is available for a new file */
{
	UINT i;
...
UINT inc_lock (	/* Increment file open counter and returns its index (0:int error) */
	...
	int32_t acc		/* Desired access mode (0:Read, !0:Write) */
)
{
	UINT i;
...
static
int32_t pick_lfn (			/* 1:Succeeded, 0:Buffer overflow */
...
FRESULT dir_read (
	DIR *dj,		/* Pointer to the directory object */
	int32_t vol			/* Filtered by 0:file/dir or 1:volume label */
)
{
	FRESULT res;
	BYTE a, c, *dir;
...
)
{
	...
	for (cc = res = 0; cc != EOF; res += cc) {
		...
		v = (f & 4) ? (ULONG)va_arg(arp, int32_t) : ((d == 'D') ? (ULONG)(int32_t)va_arg(arp, int32_t) : (ULONG)va_arg(arp, uint32_t));
		if (d == 'D' && (v & 0x80000000)) {
			v = 0 - v;
			f |= 8;
		}
