...
class SDIO_Waiter : public GenericIOInterface{
public:
    virtual bool isWriteFinished(){
		return true;
    }
    virtual bool isDataReady(){
    	return (DMAEndOfTransfer != 0x00) || (TransferEnd != 0) || (TransferError != SD_OK);
    }

    bool waitForOp(){
    	int64_t timeout = NOW() + SD_DATATIMEOUT_MS*MILLISECONDS;
    	while(!isDataReady()){
    		suspendUntilDataReady(timeout);
...
SD_Error SD_WaitReadOperation(void)
{
  SD_Error errorstatus = SD_OK;
   ...
   if(SDIO_Waiter_instance.waitForOp()){
	  SD_StopTransfer();
	  SDIO_ClearFlag(SDIO_STATIC_FLAGS);
...
void SDIO_IRQHandler(void)
{
  ...
  SD_ProcessIRQSrc();
  SDIO_Waiter_instance.upCallDataReady();
}
